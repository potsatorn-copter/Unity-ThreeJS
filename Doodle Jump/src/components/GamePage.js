...
import { updateScoreDisplay, removeScoreDisplay } from './ScoreDisplay'; // จัดการการแสดงผลคะแนน
...
const GamePage = () => {
    ...
    const timerRef = useRef(null); // ตัวแปร ref สำหรับ timer
    const keys = useRef({
        a: { pressed: false },
        d: { pressed: false }
    }).current;
    const jumpSoundRef = useRef(); // ตัวแปร ref สำหรับเสียงกระโดด
    const breakSoundRef = useRef(); // ตัวแปร ref สำหรับเสียงแพลตฟอร์มแตก
    const winSoundRef = useRef(); // ตัวแปร ref สำหรับเสียงชนะ
    const gameOverSoundRef = useRef(); // ตัวแปร ref สำหรับเสียงจบเกม
    const characterRef = useRef(null); // ตัวแปร ref สำหรับตัวละคร
    const hasGameEnded = useRef(false); // ตัวแปร ref สำหรับสถานะเกมจบ
    const updateDebugInfo = (newInfo) => {
        setDebugInfo(prevDebugInfo => ({ ...prevDebugInfo, ...newInfo }));
    };
    const navigate = useNavigate();
    let scene; // ตัวแปรสำหรับ scene
    let camera; // ตัวแปรสำหรับ camera
    let renderer; // ตัวแปรสำหรับ renderer
    let isRunning; // ตัวแปรสำหรับสถานะการทำงานของเกม
    const isJumping = useRef(false); // ตัวแปร ref สำหรับสถานะการกระโดด
    let highestPlatformY; // ตัวแปรสำหรับตำแหน่ง Y ของแพลตฟอร์มที่สูงที่สุด
    ...
    let platforms = []; // ตัวแปรสำหรับเก็บแพลตฟอร์ม
    ...
    function resetGame() {
        ...
        if (characterRef.current) {
            ...
            characterRef.current.velocity = { x: 0, y: jumpVelocity, z: 0 };
        }
    ...
    function animate() {
        ...
        updateDebugInfo({
            ...
            velocity: characterRef.current?.velocity,
        ...
        if (characterRef.current) {
            ...
            if (isJumping.current) {
                ...
                checkPlatformCollision(characterRef, platforms, scene, highestPlatformY, maxGap, minHeightRef, isJumping, scoreRef, jumpVelocity, gravity, jumpSoundRef, breakSoundRef, updateDebugInfo, endGame, createNewPlatform);
            }
            else {
    ...
    return (_jsx("div", { children: isWin || isGameOver ? (_jsx(Leaderboard, { latestScore: latestScore, isWin: isWin, latestPlayerName: playerName, latestCharacter: selectedCharacter })) : (_jsxs(_Fragment, { children: [_jsx("div", { id: "button-container", className: "d-none text-center" }), _jsxs("div", { id: "message-container", className: "message-container text-center", children: [_jsx("div", { id: "message", className: "message" }), _jsx("button", { id: "restartButton", className: "btn btn-primary btn-lg m-2 d-none", children: "Restart Game" })] }), _jsxs("div", { className: "timer", children: ["Time left: ", timeLeft, " seconds"] }), _jsx(DebugOverlay, { debugInfo: debugInfo })] })) }));
};
