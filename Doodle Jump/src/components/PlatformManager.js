...
export function createPlatform(x, y, z, color, scene, platforms) {
    const platform = new THREE.Mesh(new THREE.BoxGeometry(10, 0.7, 10), // สร้าง BoxGeometry สำหรับ platform
    ...
    platform.userData.isDestructible = color === '#ff0000'; // กำหนดว่าถ้าเป็นสีแดงจะทำลายได้
...
export function createNewPlatform(highestPlatformY, maxGap, platforms, scene, createBluePlatformNearby) {
    ...
    let newZ = undefined; // ตำแหน่ง Z ของแพลตฟอร์มใหม่
    while (!isPositionValid && attempts < 100) {
        ...
        for (let i = 0; i < platforms.length; i++) {
            ...
            if (otherPlatform.position.y > newY - maxGap && otherPlatform.position.y < newY + maxGap) {
                if (otherPlatform.material.color.getHex() === 0x0369a1) {
                    isBluePlatformNearby = true; // ตรวจสอบว่ามีแพลตฟอร์มสีน้ำเงินอยู่ใกล้ๆ
                }
                if (otherPlatform.material.color.getHex() === 0xff0000) {
...
export function checkPlatformCollision(characterRef, platforms, scene, highestPlatformY, maxGap, minHeightRef, isJumping, scoreRef, jumpVelocity, gravity, jumpSoundRef, breakSoundRef, updateDebugInfo, endGame, createNewPlatform) {
    // เช็คชนแพลตฟอร์มในส่วนด้านบนของแพลตฟอร์ม
    platforms.forEach(platform => {
        const boxGeometry = platform.geometry;
        if (characterRef.current.position.y <= platform.position.y + 0.5 &&
